import { SDKVersion, SnackDependencies, SnackDependencyVersions, SnackFiles, SnackMissingDependencies } from 'snack-content';
...
export declare type SnackConnectedClientStatus = 'ok' | 'error' | 'reloading';
/**
 * Client which connected to the Snack.
 *
 * Clients are only able to connect when the Snack is `online`.
 */
export declare type SnackConnectedClient = {
    transport: string;
    id: string;
    name: string;
    platform: string;
 ...
 */
export declare type SnackSendBeaconRequest = {
    url: string;
    data: any;
};
export declare type SnackListenerSubscription = () => any;
/**
 * Log data for when a connected client calls one of the
 * `console.[log,info,warn,error]` to log information to the
 * console.
 */
export declare type SnackLogEvent = {
    type: 'error' | 'warn' | 'log' | 'info';
    message: string;
    connectedClient?: SnackConnectedClient;
    error?: SnackError;
};
export declare type SnackWindowRef = {
    current: Window | null;
};
export declare type SnackState = {
    /**
     * Expo SDK version.
     */
